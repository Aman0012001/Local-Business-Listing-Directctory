    ...
    try {
        ...
        for (const user of usersToFix) {
            ...
            if (res.rows.length > 0) {
                ...
                await client.query('UPDATE users SET password = $1, role = $2, is_active = true, is_email_verified = true WHERE email = $3', [hash, user.role, user.email]);
                console.log(`Updated ${user.email}.`);
            }
            else {
                console.log(`User ${user.email} not found. Creating user...`);
                await client.query('INSERT INTO users (id, email, password, full_name, role, is_active, is_email_verified) VALUES (uuid_generate_v4(), $1, $2, $3, $4, true, true)', [user.email, hash, user.email.split('@')[0], user.role]);
