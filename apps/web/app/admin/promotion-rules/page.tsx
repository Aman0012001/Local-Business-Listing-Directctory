...
import {
    Zap, Home, LayoutGrid, Info, Save,
    Loader2, CheckCircle2, XCircle,
    ArrowRight, Settings, CreditCard,
    TrendingUp, ShieldCheck, MinusSquare,
    Tag, Calendar, Layout
} from 'lucide-react';
...
const PLACEMENT_INFO = {
    ...
    offer: {
        title: 'Offer Boost',
        description: 'Set baseline for time-limited deal promotions across the app.',
        icon: Tag,
    ...
    event: {
        ...
        description: 'Control baseline cost for increasing event and workshop reach.',
        icon: Calendar,
        color: 'bg-rose-600',
...
export default function PromotionRulesPage() {
    ...
    const handleUpdate = async (id: string, updates: Partial<PricingRule>) => {
        setSavingId(id);
        try {
            const updated = await api.patch<PricingRule>(`/promotions/admin/rules/${id}`, updates);
        ...
        return (
            <div className="flex items-center justify-center py-32">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-400 font-bold animate-pulse">Syncing Monetization Rules...</p>
                </div>
            </div>
        );
    }
    ...
    const avgRate = rules.length > 0
        ? Math.round(rules.reduce((acc, curr) => acc + Number(curr.pricePerHour), 0) / rules.length)
        : 0;
        ...
        <div className="max-w-6xl mx-auto space-y-12 pb-32">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-black uppercase tracking-[0.3em] text-red-500">Monetization Engine</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-none">Promotion Boost Pricing</h1>
                        <p className="text-slate-400 font-bold text-lg max-w-md">
                            Configure global hourly rates for vendor promotions and set placement visibility.
                        </p>
                    </div>

                    <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
                        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10">
                            <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Active Zones</div>
                            <div className="text-3xl font-black">{activeRules} / {rules.length}</div>
                        </div>
                        <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10">
                            <div className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Avg. Hourly Rate</div>
                            <div className="text-3xl font-black">{avgRate} <span className="text-sm text-slate-500 tracking-normal text-slate-500 font-normal">PKR</span></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    ...
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">Dynamic Pricing V2.0</div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {rules.map((rule) => {
                        const info = PLACEMENT_INFO[rule.placement as keyof typeof PLACEMENT_INFO] || PLACEMENT_INFO.listing;
                            ...
                            >
                                ...
                                <div className={`${info.lightColor} p-8 relative`}>
                                    <div className="absolute top-4 right-4">
                                        <button
                                            onClick={() => handleUpdate(rule.id, { isActive: !rule.isActive })}
                                    ...
                                    <div className={`w-16 h-16 rounded-[20px] ${info.color} flex items-center justify-center text-white shadow-xl mb-6 group-hover:rotate-6 transition-all duration-300`}>
                                        <info.icon className="w-8 h-8" />
                                    </div>
                                    <h3 className="text-2xl font-black text-slate-900 mb-2 leading-none">{info.title}</h3>
                                    <p className="text-sm font-bold text-slate-500 leading-relaxed font-semibold">{info.description}</p>
                                </div>
                                ...
                                <div className="p-8 space-y-8">
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex justify-between items-end">
                                                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Hourly Boost Rate</label>
                                                <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-2 py-0.5 rounded-full">Automated Multiplier</span>
                                            </div>
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                                        <div className="flex flex-col">
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Estimated Daily</span>
                                            <span className="text-lg font-black text-slate-900">Rs {Number(rule.pricePerHour) * 24}</span>
                                        </div>
                                        <button
                                            disabled={isSaving}
                                        ...
                                        >
                                            {isSaving ? <Loader2 className="w-6 h-6 animate-spin text-slate-400" /> : <Save className="w-6 h-6" />}
                                        </button>
                                    </div>
                                </div>
                            </motion.div>
                    ...
                    {rules.length === 0 && (
                        <div className="col-span-full py-20 flex flex-col items-center justify-center bg-white rounded-[32px] border-2 border-dashed border-slate-100">
                            ...
                            <h3 className="text-xl font-black text-slate-400 uppercase tracking-widest">No Rules Initialized</h3>
